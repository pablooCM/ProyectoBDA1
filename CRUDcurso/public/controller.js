...
$scope.desseleccionarPreferencias= function(){
  $scope.preferencia = "";
}

$scope.addPreferencia = function(){
  console.log("ENTRE");  
  $http.get('/listaDeCursos/'+ $scope.cursoid).success(function(response){
    console.log("SECDFVGVBHGFDCEXCFVGBHGFGRDCEVFGBHNJ");
    response.preferencia.push($scope.preferencia);
    $http.put('/listaDeCursos/'+cursoid,response).success(function(respuesta){
          refresh();
    });
  });
}

$scope.removePreferencia = function(cursoid){
  console.log($scope.cursoid); 
  $http.get('/listaDeCursos/'+ id).success(function(response){
    var pre =response.preferencia.indexOf($scope.preferencia)
     response.preferencia.splice(pre,1);
    $http.put('/listaDeCursos/:_id/:pref', function(req, res,preferencia){
      refresh();
    });
  });
};

$scope.actualizarPreferencia = function(cursoid){
  $http.get('/listaDeCursos/'+ $scope.cursoid).success(function(response){
    var pre =response.preferencia.indexOf($scope.preferencia);
    response.preferencia[pre] = $scope.preferencia;
    $http.put('/listaDeCursos/:_id/:pref', function(req, res,preferencia){
      refresh();
    });
  });
